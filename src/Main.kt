...
fun adicionarNota(materia: String, nota: Double): Boolean {
    val notasDaMateira = materiasENotas[materia]

    return if(notasDaMateira != null) {
        notasDaMateira.add(nota)
        true
    } else {
        false
    }
...
fun mostrarNotas(materia: String) {
    ...
    if (listaNotas != null) {
        ...
        println("Não foi possível mostrar as notas da matéira $materia")
    }
}
...
fun main(){
    ...
    val notas = mutableListOf<Double>(30.0, 55.7, 60.0)
    adicionarDisciplina("PWEB2", notas);

    // 2. adicionarDisciplinas -> adicione 1 disciplina ao map materiasENotas, através de atribuição nomeada
    adicionarDisciplina(materia = "Inglês", notas = notas);
